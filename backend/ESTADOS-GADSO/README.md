...
- Windows 10/11 o Linux/Coder.
- Python instalado y disponible en consola con `python --version`. Para Linux/Coder se recomienda Python 3.11 o 3.12.
...
Instalacion recomendada en Linux/Coder:
...
Si el contenedor base no trae dependencias del navegador:
...
Forma recomendada en Linux/Coder:

```bash
chmod +x run_agents_flow.sh
./run_agents_flow.sh
./run_agents_flow.sh JV
./run_agents_flow.sh SELVA
./run_agents_flow.sh TODOS
./run_agents_flow.sh JV --solo-login
```
